<template>
	<div class="jiashi_container">
		<div class="jiashi_top">
			<i class="iconfont icon-icon goBack" @click="goBack"></i>
			<p class="jiashi_song">居家软饰</p>
			<!-- <span class="iconfont icon-mp-search sear" @click="searchClick"></span> -->
		</div>
		<div class="jiashi_main">
			<div class="jiashi_banner">
				<img src="src/assets/imgs/home_jiayong.jpeg" alt="">
			</div>
			<!-- <ul>
				<li v-for="(arr,index) in imgs" >
					<router-link :to="arr[1]">
						<img :src="'src/assets/imgs/'+arr[0]" alt="" class="jiashi_imgs">
					</router-link>
				</li>
			</ul> -->
			<div class="jiashi_goods">
				<div id="topNav" class="swiper-container jiashi_nav">
				    <div class="swiper-wrapper">
				    	<div class="swiper-slide a e2" v-for="(arr,index) in nav" :class="{'c3':activeIndex === index}" @click="navClick(index)">{{arr}}</div>
				    </div>
				</div>
				...
				<ul>
					...
					<router-link :to="'details?id='+ arr.id">
						<img :src="'src/assets/imgs/'+ JSON.parse(arr.banner)[0]" alt=""   class="jiashi_goodsimg">
						<span class="jiashi_goodstitle">{{arr.ProductName}}</span>
					</router-link>
       					<span class="jiashi_nowprice">{{arr.NewPrice}}<span class="idnex_oldprice">{{arr.OldPrice}}</span></span>
       					<i class="jiashi_car iconfont icon-gouwuche" @click="addCart(index)"></i>
					</li>
				</ul>
				<div class="jiashi_no-more">没有更多的商品了</div>
