<template>
	<div class="chufang_container">
		<div class="chufang_top">
			<i class="iconfont icon-icon goBack" @click="goBack"></i>
			<p class="chufang_song">创意厨房</p>
			...
			<!-- <ul>
				<li v-for="(arr,index) in imgs" >
					<router-link :to="arr[1]">
						<img :src="'src/assets/imgs/'+arr[0]" alt="" class="chufang_imgs">
					</router-link>
				</li>
			</ul> -->
			<div class="chufang_goods">
				<div id="topNav" class="swiper-container chufang_nav">
				...
				<ul>
					...
					<router-link :to="'details?id='+ arr.id">
						<img :src="'src/assets/imgs/'+ JSON.parse(arr.banner)[0]" alt=""   class="chufang_goodsimg">
						<span class="chufang_goodstitle">{{arr.ProductName}}</span>
					</router-link>
       					<span class="chufang_nowprice">{{arr.NewPrice}}<span class="idnex_oldprice">{{arr.OldPrice}}</span></span>
       					<i class="chufang_car iconfont icon-gouwuche" @click="addCart(index)"></i>
					</li>
				</ul>
				<div class="chufang_no-more">没有更多的商品了</div>
